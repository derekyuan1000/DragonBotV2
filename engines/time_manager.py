...
def calculate_time_allocation(board: chess.Board, time_limit: chess.engine.Limit, max_depth: int) -> Tuple[float, int]:
    opening_time_factor = 0.05
    middlegame_time_factor = 0.1
    endgame_time_factor = 0.12
    increment_usage_factor = 0.8
    position_complexity_weight = 1.0

    def evaluate_position_complexity(b: chess.Board) -> float:
        complexity = 0.0
        ...
        capture_count = sum(1 for m in legal_moves if b.is_capture(m))
        complexity += capture_count * 1.5

        if b.is_check():
            complexity += 8.0

        check_count = sum(1 for m in legal_moves if b.gives_check(m))
    ...
    def calculate_game_phase(b: chess.Board) -> str:
        piece_count = len(b.piece_map())
        move_count = len(b.move_stack)
    ...
    if game_phase == "opening":
        time_factor = opening_time_factor
        base_depth = 7
    elif game_phase == "middlegame":
        time_factor = middlegame_time_factor
        base_depth = 9
    else:
        time_factor = endgame_time_factor
        base_depth = 6

    complexity = evaluate_position_complexity(board)
    complexity_normalized = min(complexity / 30.0, 2.0)

    adjusted_time_factor = time_factor * (1.0 + complexity_normalized * position_complexity_weight)
    ...
    allocated_time += increment * increment_usage_factor
