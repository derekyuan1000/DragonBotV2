...
class DragonBot(MinimalEngine):
        ...
        if os.path.basename(path).lower() == "openings.csv":
            ...
            if not os.path.isabs(path):
                candidate_paths.append(
                    os.path.normpath(os.path.join(os.path.dirname(__file__), "..", dir_part, "openings.csv")))
            ...
            try:
                with open(p, "r", encoding="utf-8") as f:
                    ...
                    for row in reader:
                        ...
                        if parsed_moves:
                            self.csv_openings.append(
                                Opening(eco_code=eco, name=name or eco or "Opening", moves=parsed_moves))
    ...
    def _find_csv_opening_move(self, board: chess.Board) -> Optional[Tuple[chess.Move, str]]:
        if not self.use_csv_openings or not self.csv_openings:
            return None
    ...
    def _tb_in_range(self, board: chess.Board) -> bool:
        return chess.popcount(board.occupied) <= self.tb_probe_limit and board.uci_variant == "chess"
        ...
        for path in self.book_paths:
            try:
                with chess.polyglot.open_reader(path) as reader:
                    ...
                    entries = [e for e in entries if e.weight >= self.book_min_weight]
                    if not entries:
                        continue
                    if self.randomize_book:
